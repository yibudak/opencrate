...
- **Power:** Windows power plans including Ultimate Performance, live CPU metrics, Quiet/Everyday/Responsive processor presets, advanced boost and energy controls, and undo for the last change. Plugged-in and battery controls appear only on battery-powered computers; desktops and UPS systems use AC settings.
...
Power control uses Windows power-policy APIs. Available settings depend on Windows, hardware and permissions. Ultimate Performance reuses an installed template copy when recognized by its Windows-localized name or known GUID. Renamed custom copies remain available in the plan selector. If absent, clicking Ultimate Performance creates one persistent copy using the Windows template and activates it; Windows may reject this on restricted systems. Undo restores the previous active plan and leaves the installed copy available.

CPU telemetry refreshes about once per second while the Power page is visible, with a 60-second history. Run [LibreHardwareMonitor](https://github.com/LibreHardwareMonitor/LibreHardwareMonitor/releases) or [OpenHardwareMonitor](https://openhardwaremonitor.org/) as administrator with WMI publishing enabled for package power, temperature and core clocks. OpenCrate reads their local `ROOT\\LibreHardwareMonitor` / `ROOT\\OpenHardwareMonitor` sensor providers and reconnects automatically within 15 seconds. These tools are separate prerequisites and are not bundled. Without a sensor provider, Windows performance counters supply usage and an explicitly labeled frequency estimate. Missing or expired measurements display a dash. OpenCrate never substitutes board temperature, GPU power, TDP or base clock for a live CPU measurement. Sensor updates occur in a separate worker and cannot block power-plan actions.

Quiet, Everyday and Responsive prepare Windows processor-policy drafts for review and Apply; they do not change AMD firmware modes. Processor percentages are performance policies, not CPU watt limits. Ryzen systems also show available core-voltage/TDC/EDC readings and a Ryzen Master launcher or download link. PBO, Curve Optimizer, AMD Eco Mode and PPT/TDC/EDC **limits** must be adjusted in Ryzen Master or UEFI/BIOS: [AMD's public monitoring SDK](https://www.amd.com/en/developer/ryzen-master-monitoring-sdk.html) only provides read access. OpenCrate does not write CPU voltage, SMU registers or firmware limits.
...
The UI uses event-driven egui with cached software rasterization and GDI presentation. It does not initialize a GPU rendering device; closing, minimizing or occluding the window drops rendering surfaces and frame buffers. Hardware polling and protection continue independently of drawing.

## Settings and application lifetime

Preferences are stored in `%APPDATA%\opencrate\settings.json`. They include `language` (`en`, `zh-CN` or `tr`), `theme` (`system`, `light` or `dark`), tray startup, lighting restoration and the last successfully applied lighting configuration. Existing files without a language field continue to work in English; an unknown language also falls back to English. Missing or unknown theme values follow the system theme.
